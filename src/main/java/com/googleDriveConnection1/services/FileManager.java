...
public class FileManager {
	...
	public List<File> listEverything() throws IOException, GeneralSecurityException {
		...
		FileList result = googleDriveManager.getInstance().files().list().setPageSize(10)
	...
	public List<File> listFolderContent(String parentId) throws IOException, GeneralSecurityException {
		...
		try {
			...
			result = googleDriveManager.getInstance().files().list().setQ(query).setPageSize(10)
					.setFields("nextPageToken, files(id, name)").execute();
	...
	public void downloadFile(String id, OutputStream outputStream) throws IOException, GeneralSecurityException {
		String filePath = "C:\\New folder\\output_file3.pdf";
		if (id != null) {
			String fileId = id;
			try (OutputStream outputStream2 = new FileOutputStream(filePath)) {
				googleDriveManager.getInstance().files().get(fileId).executeMediaAndDownloadTo(outputStream2);
			} catch (IOException e) {
				e.printStackTrace();
				System.err.println("Error creating or using the OutputStream.");
			}
		}
	...
	public String uploadFile(MultipartFile file, String filePath) {
		try {
			...
			if (null != file) {
				...
				File uploadFile = googleDriveManager.getInstance().files().create(fileMetadata,
